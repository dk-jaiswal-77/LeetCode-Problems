...
class Solution {
    ...
    private void inTraversal(TreeNode root, List<Integer> res) {
        if (root == null) return;
        ...
        boolean threadExists = doesThreadExists(root.left, root);
        if (threadExists) {
            // thread exists --> traversed. so cut the thread and traverse right
            cutThread(root.left, root);
            res.add(root.val);
            inTraversal(root.right, res);
            return;
        }
        ...
        createThread(root.left, root);
        // findLeftSubTreeLastNode(root.left).right = root;
